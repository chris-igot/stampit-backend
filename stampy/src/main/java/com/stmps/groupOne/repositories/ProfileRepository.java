...
public interface ProfileRepository extends CrudRepository<Profile,String> {
	...
	@Query(value = "SELECT profiles.id as id, profiles.name as name, profiles.image_id as image_id, profiles.title as title, profiles.bio as bio, profiles.is_private as is_private, profiles.user_id as user_id, profiles.created_at as created_at, profiles.updated_at as updated_at FROM profiles INNER JOIN follows ON follows.you_id = profiles.id WHERE follows.them_id = :profileId AND follows.follow_verified IS TRUE;", nativeQuery = true)
