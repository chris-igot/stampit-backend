			...
			<c:choose>
				...
				<c:when test="${followed == true }">
					<a class="btn btn-outline-dark"
						href="/profile/unfollow?id=${profile.id }">Followed (<c:out
							value="${profile.followers.size() }" />)
					</a>
				</c:when>
				<c:when test="${followed == false }">
					<a class="btn btn-warning" href="/profile/follow?id=${profile.id }">Follow
						(<c:out value="${profile.followers.size() }" />)
					</a>
				</c:when>
			</c:choose>
		</div>
			...
			<c:forEach items="${profile.posts}" var="post">
				<div class="col-4 card mb-1">
					<a href="/post?id=${post.id}"> <img
						src="/img/${post.image.fileName }" class="card-img-bottom" alt="" />
					</a>
				</div>
			</c:forEach>
		</div>
